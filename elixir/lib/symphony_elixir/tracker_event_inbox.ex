defmodule SymphonyElixir.TrackerEventInbox do
  ...
  alias SymphonyElixir.{RunLedger, TrackerEvent}
  ...
  def enqueue(events) when is_list(events) do
    ...
    {records, next_state, duplicates} =
      Enum.reduce(events, {[], state, 0}, fn
        %TrackerEvent{} = event, {acc, state_acc, duplicate_count} ->
          key = TrackerEvent.dedupe_key(event)
          ...
          if seen_dedupe_key?(state_acc, key) do
            ...
            {
              [record | acc],
              remember_dedupe_key(state_acc, key),
              duplicate_count
            }
          end
  ...
  @spec stats() :: %{
          ...
          oldest_pending_event_at: String.t() | nil
        }
  def stats do
  ...
  defp generate_event_id do
    "trk_" <> Base.url_encode64(:crypto.strong_rand_bytes(9), padding: false)
  end
