...
## Next Step
Restart canary on the focused prompt-hardening patch, trigger `CLZ-22` again, and confirm the live run clears both the old three-turn implement stop and the prior `142,060` high-pressure per-turn-input failure without emitting a follow-on `implementation.invalid_turn_result`. After that, continue `CLZ-22` by replacing the stable-ingress relay bridge with a durable scheduler/assignment seam so stable persists cross-runner work instead of forwarding raw webhook requests.
