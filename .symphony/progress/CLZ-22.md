...
## Next Step
Continue `CLZ-22` by implementing the missing stable-ingress scheduler seam so the stable process can accept GitHub review webhooks and assign them to the healthy canary runner without GitHub pointing at canary directly. After that, add stronger proof adapters for the newly-pending PR comments and richer promotion safety checks around canary evidence and rollback policy.
