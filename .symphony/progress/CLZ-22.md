...
## Next Step
Restart canary on the dedicated scoped review-fix turn-budget patch, trigger `CLZ-22` again, and confirm the live run advances past the old `implementation.turn_budget_exhausted` stop into further scoped edits or normal validation/review follow-up. After that, continue `CLZ-22` by replacing the stable-ingress relay bridge with a durable scheduler/assignment seam so stable persists cross-runner work instead of forwarding raw webhook requests.
