...
## Next Step
Restart canary on the scoped-turn-window patch, trigger `CLZ-22` again, and confirm the live run now advances past both the old `60,000` soft-budget regression and the inherited `implementation.turn_budget_exhausted` stop. If the run still blocks, capture the next live ledger event and continue narrowing the remaining autonomous PR-comment handoff until the canary finishes the open GitHub review claims end to end.
