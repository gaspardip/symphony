...
## Next Step
Restart canary on the tightened high-pressure command-budget patch, trigger `CLZ-22` again, and confirm the live run stays below the recovered per-turn input cap instead of repeating the `170,779` budget stop. After that, continue `CLZ-22` by replacing the stable-ingress relay bridge with a durable scheduler/assignment seam so stable persists cross-runner work instead of forwarding raw webhook requests.
